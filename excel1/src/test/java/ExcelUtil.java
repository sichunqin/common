...
public class ExcelUtil {
    ...
    public static void copyMergedRegion(Sheet srcSheet, Row srcRow, Row desRow) {
        int sheetMergerCount = srcSheet.getNumMergedRegions();
        for (int i = 0; i < sheetMergerCount; i++) {
            if(srcSheet.getMergedRegion(i).getFirstRow() == srcRow.getRowNum()){
                CellRangeAddress range = srcSheet.getMergedRegion(i);
                range.setFirstRow(desRow.getRowNum());
                range.setLastRow(desRow.getRowNum());
                srcSheet.addMergedRegion(range);
            }
        }
    }
